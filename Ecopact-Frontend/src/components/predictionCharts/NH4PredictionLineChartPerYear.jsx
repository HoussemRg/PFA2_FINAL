...
const NH4PredictionLineChartPerYear = (props) => {
  const {NH4PredictionDataPerYear}=useSelector(state => state.prediction);
  const months=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
     ...
     const data=calculateAvg(NH4PredictionDataPerYear);
     const finalData={
      labels: data.map(item => item.month),
        datasets: [
          {
            label: "NH4 Prediction Average Rates",
      ...
      const options={
        plugins: {
          title: {
            display: true,
            text: "NH4 Prediction Average Rates in 2024"
          },
          legend: {
            display:false
          }
...
export default NH4PredictionLineChartPerYear
