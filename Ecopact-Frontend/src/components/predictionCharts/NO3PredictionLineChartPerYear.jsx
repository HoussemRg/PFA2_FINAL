...
const NO3PredictionLineChartPerYear = (props) => {
    const {NO3PredictionDataPerYear}=useSelector(state => state.prediction);
    const months=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
       ...
       const data=calculateAvg(NO3PredictionDataPerYear);
       const finalData={
        labels: data.map(item => item.month),
          datasets: [
            {
              label: "NO3 Prediction Average Rates",
        ...
        const options={
          plugins: {
            title: {
              display: true,
              text: "NO3 Prediction Average Rates in 2024"
            },
            legend: {
              display:false
            }
...
export default NO3PredictionLineChartPerYear
