...
const PxOyPredictionLineChartPerYear = (props) => {
  const {PxOyPredictionDataPerYear}=useSelector(state => state.prediction);
  const months=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
     ...
     const data=calculateAvg(PxOyPredictionDataPerYear);
    
     const finalData={
      labels: data.map(item => item.month),
        datasets: [
          {
            label: "PxOy Prediction Average Rates",
      ...
      const options={
        plugins: {
          title: {
            display: true,
            text: "PxOy Prediction Average Rates in 2024"
          },
          legend: {
            display:false
          }
...
export default PxOyPredictionLineChartPerYear
